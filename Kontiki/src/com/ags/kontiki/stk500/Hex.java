...
/**
 * Class for interpreting Intel hex files and verifying the binary code. Use
 * {@link #getHexLine(int, int) getHexLine} to request chunks of bytes.
 */
public class Hex {
	...
	public Hex(byte[] bin) {
		...
		Log.v("Hex", "Hex file status: " + state);
	}
	...
	public byte[] getHexLine(int startByte, int numberOfBytes) {
		try {
			Log.d("Hex", "Hex.getHexLine: startByte: " + startByte + ", numberOfBytes: " + numberOfBytes);
			return formatHexLine(startByte, numberOfBytes);
		} catch (IndexOutOfBoundsException e) {
			// There was no bytes, return an empty array
			Log.w("Hex", "Hex.getHexLine: startByte is out of bounds! Value was: " + startByte + ", max value: " + dataList.size());
	 ...
	 *             When <code>startByte</code> does not exist.
	 */
	private byte[] formatHexLine(int startByte, int numberOfBytes) throws IndexOutOfBoundsException {
		...
		if ((startByte + numberOfBytes) > dataList.size()) {
			...
			Log.i("Hex", "Hex.formatHexLine: Could not read " + numberOfBytes + " bytes, changed to " + dataLength);
		}
	...
	/**
	 * Split the hex input into an array and check if it's correct. Each line
	 * must start with ':' (colon), byte value 58. The following values is 1
	 * byte size, 2 byte address, n byte data, 1 byte checksum. The record is
	 * not saved, only used to check what kind of data this is. This will start
	 * with line 0 and parse through the whole file.
	 * 
	 * @return True if the hex file is correct.
	 */
	 ...
	 * must start with ':' (colon), byte value 58. The following values is 1
	 * byte size, 2 byte address, n byte data, 1 byte checksum. The record is
	 * not saved, only used to check what kind of data this is.
	 * 
	 * @param startOnDataByte
	 *            Start reading at this byte number. When finished with this
	 *            line, starting on the next line.
	 * 
	...
	private int splitHex(int startOnDataByte) {
		...
		if ((subHex.length + startOnDataByte) < 6) {
			Log.w("Hex", "splitHex(): The minimum size of a line is 6, this line was " + subHex.length);
			return -1;
		}
		...
		if (subHex[startOnDataByte] != 58) {
			Log.w("Hex", "splitHex(): Line not starting with ':' !");
			return -1;
		}
		// If record type is 0x01 (file end) and data size > 0, return false
		else if (subHex[startOnDataByte + 4] == 1 && dataLength > 0) {
			Log.w("Hex", "splitHex(): Contains data, but are told to stop!");
			return -1;
		}
		// If record type is 0x01 (file end) and it exist more bytes to read,
		// return false
		else if (subHex[startOnDataByte + 4] == 1 && subHex.length > startOnDataByte + dataLength + 6) {
			Log.w("Hex", "splitHex(): Contains more lines with data, " + "but are told to stop!");
			return -1;
		}
		// If record type is 0x00 (data record) and data size equals 0, return
		// false
		else if (subHex[startOnDataByte + 4] == 0 && subHex[startOnDataByte + 1] == 0) {
			Log.w("Hex", "splitHex(): Told to send data, but contains no data!");
			return -1;
		} else {
			...
			// Checksum not correct
				...
				Log.w("Hex", "splitHex(): Checksum failed!");
				return -1;
			}
		}
	}
	...
	/**
	 * Calculate and check the checksum of the given byte array with the
	 * checksum from hex file.
	 * 
	 * @param data
	 *            Byte array to check.
	 * @param startByte
	 *            Where to start loading bytes from hex file.
	 * 
